...
export class EmployeeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.signalrConnectionEstablished$ = this._employeeService.connectionEstablished$;
    this._employeeService.priorityNumberRecieved$.subscribe(message => {
      this.destroyTable();
      this.getEmployeeList(true);
    });
  }

  getEmployeeList(isSignalR: boolean = false) {
    if (!isSignalR) {
      ...
      this._employeeService.getEmployeesWithoutLoading().then((res) => {
        if (res.isSuccess) {
          this.employeeList = res.data;
          this.loadDataTable();
        }
